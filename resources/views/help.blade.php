...
<div>
    <div class='py-24 bg-grad-pink h-screen w-full px-2 flex flex-col items-center justify-center text-sm px-6 relative overflow-hidden'>
        ...
        <h1 class="text-white text-2xl self-start z-10 font-bold text-center m-auto">Halo {{ $customer->name }},</h1>
        ...
        <div class='flex flex-col items-start gap-3 z-10 overflow-y-scroll h-96' id="faq-items">
            <div class='border border-white bg-white shadow-c-25 w-full rounded-xl py-3 px-2'>
...
<script>
    $(document).ready(function() {
        $('#inputQuery').on('input', function() {
            ...
            $('#faq-items div').filter(function() {
                var faqText = $(this).text().toLowerCase();
                return faqText.includes(searchQuery);
            }).show();
            $('#faq-items div').filter(function() {
                var faqText = $(this).text().toLowerCase();
                return !faqText.includes(searchQuery);
